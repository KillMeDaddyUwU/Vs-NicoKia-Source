...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg = new FlxSprite().loadGraphic(Paths.image('stagebackBlur'));
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		bg.updateHitbox();
		bg.screenCenter();
		bg.x -= 250;
		FlxTween.tween(bg, {x: bg.y + 225}, 35, {ease: FlxEase.linear, type: PINGPONG});
		
		add(bg);

		logoBl = new FlxSprite().loadGraphic(Paths.image('Logo screen'));
		...
		logoBl.angle = -25;
		FlxTween.tween(logoBl, {y: logoBl.y + 25}, 1.8, {ease: FlxEase.quadInOut, type: PINGPONG});
		FlxTween.tween(logoBl, {angle: 25}, 5.4, {ease: FlxEase.quadInOut, type: PINGPONG});
		FlxTween.tween(logoBl.scale, {x: logoBl.scale.x + 0.025,y: logoBl.scale.y + 0.025}, 3.6, {ease: FlxEase.quadInOut, type: PINGPONG});
